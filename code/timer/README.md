# Overview
This example expands on the blink example to make the LED turn on and off with a non-blocking timer. It puts Timer/Counter Control Register A in *Clear Timer on Compare* (CTC) mode. When the clock value "overflows", an interrupt is triggered prompting our program in increment the milliseconds counter.
